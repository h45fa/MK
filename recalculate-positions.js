...
function recalculateFighterPosition(fighter, opponent) {
	...
	if (
		(!fighter.isJumping() && opponent.isJumping()) ||
		(fighter.isJumping() && !opponent.isJumping())
	) {
		return
	}
	...
	const haveXCollision =
		Math.abs(opponent.x - fighter.x) < (opponent.width + fighter.width) / 2
	...
	const collisionWidth =
		(opponent.width + fighter.width) / 2 - Math.abs(opponent.x - fighter.x)
