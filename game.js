	...
	async loadFighterImages() {
		...
		for (let moveKey in MOVE_TYPES) {
			...
			for (let i = 0; i < IMAGE_COUNT_BY_MOVE_TYPE[moveType]; i++) {
				urls.push(
					`./images/fighters/${this.fighters[0].name}/${ORIENTATIONS.LEFT}/${moveType}/${i}.png`
				)
				urls.push(
					`./images/fighters/${this.fighters[0].name}/${ORIENTATIONS.RIGHT}/${moveType}/${i}.png`
				)
				urls.push(
					`./images/fighters/${this.fighters[1].name}/${ORIENTATIONS.LEFT}/${moveType}/${i}.png`
				)
				urls.push(
					`./images/fighters/${this.fighters[1].name}/${ORIENTATIONS.RIGHT}/${moveType}/${i}.png`
				)
			}
		}
	...
	addHandlers() {
		document.addEventListener('keydown', event => {
			if (!this.pressed[event.code]) {
				...
				setMoves(this.fighters[0], this.fighters[1], this.pressed)
			}
		})
