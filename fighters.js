...
import { BackwardJump, ForwardJump, Jump } from './jump-moves.js'
import {
	AttractiveStandUp,
	Block,
	Endure,
...
export class Fighter {
	...
	endureAttack(damage, attackType) {
		if (this.moveType === MOVE_TYPES.BLOCK) {
			...
			attackType === MOVE_TYPES.UPPERCUT ||
			attackType === MOVE_TYPES.SPIN_KICK
		) {
	...
	setMove(newMoveType, startStep = 0) {
		...
		if (
			!INTERRUPTED_MOVE_TYPES.includes(this.moveType) &&
			this.currentMove.isContinue
		)
			return
